...
- Chrome MV3 **content-script only** (no background/popup). `public/manifest.json` injects `src/content-scripts/editable-text-area.js` from `dist/` on `https://www.linkedin.com/*`.
- `src/content-scripts/editable-text-area.ts` registers a single `MutationObserver` on `document.body`, finds editors via `DOM.SELECTORS.EDITABLE_COMMENT_BOX`, marks them with `DOM.ATTR.DATA_MUTATED`, and injects the lightbulb button.
