...
export const buildPostCommentsPrompt = (data: PostComments): string => {
  const goal = joinSections([
    "## Goal",
    "You are an assistant helping me write a LinkedIn comment.",
  ]);
  ...
  const comments =
    data.comments.length === 0
      ...
      : joinSections([
          "## Visible comments",
          data.comments
            .map((c, i) => `- ${i + 1}. ${normalizeTextBlock(c)}`)
            .join("\n"),
        ]);

  const instructions = joinSections([
    "## Instructions",
    "- Draft a thoughtful reply that matches the tone of the thread.",
    "- Keep it concise.",
  ]);
...
export const buildMessagesPrompt = (data: Messages): string => {
  const goal = joinSections([
    "## Goal",
    "You are an assistant helping me reply in a LinkedIn message thread.",
  ]);

  const context = joinSections([
    "## Context",
    `**Conversation with:** ${normalizeWhitespace(data.senderName) || "(unknown)"}`,
  ]);
